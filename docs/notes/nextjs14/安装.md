...
如果你是 Next.js 的新手，可以查看 [项目结构](/nextjs14/project-structure/) 文档，以了解应用程序中所有可能的文件和文件夹。

::: tip
 *   Next.js 现在默认带有 [TypeScript](/nextjs14/configuring/typescript/)、[ESLint](/nextjs14/configuring/eslint/) 和 [Tailwind CSS](/nextjs14/styling/tailwind-css/) 配置。
 *   你可以选择在项目根目录下使用 [`src` 目录](/nextjs14/configuring/src-directory/) 来将应用程序的代码与配置文件分开。
:::
...
*   `build`: 运行 `next build` 以构建生产环境的应用程序。
...
创建一个包含必要 `<html>` 和 `<body>` 标签的 [根布局](/nextjs14/routing/pages/) `app/layout.tsx`：
...
了解更多关于 [使用 App Router](/nextjs14/routing/defining-routes/) 的信息。
